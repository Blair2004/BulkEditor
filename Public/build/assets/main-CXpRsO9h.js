document.addEventListener("DOMContentLoaded",function(){const l=defineComponent({props:["config","selected","popup","state"],template:`
        ...
        `,data(){return{entries:[],isUpdating:!1,validation:new FormValidation}},methods:{truncateText(e,t){return e.length>t?e.substr(0,t)+"...":e},removeItem(e){this.entries=this.entries.filter(t=>t!==e),this.state.next({selectedEntries:this.entries}),this.entries.length===0&&this.popup.close()},closePopup(){this.popup.close()},clearAllSelection(){this.entries=[],this.state.next({selectedEntries:this.entries}),this.popup.close()},bulkUpdate(){if(!this.validation.validateFields(this.fields))return nsNotice.error(__m("Validation Error","BulkEditor"),__m("Please fill all the required fields","BulkEditor"));if(!Object.values(this.fields).some(i=>i.value!==""))return nsNotice.error(__m("No Changes","BulkEditor"),__m("Please make some changes before updating","BulkEditor"));Popup.show(nsConfirmPopup,{title:__m("Bulk Update Confirmation","BulkEditor"),message:__m("Are you sure you want to update {entries} selected entries?","BulkEditor").replace("{entries}",this.entries.length),onAction:i=>{i&&(this.isUpdating=!0,nsHttpClient.post("/api/bulk-update",{class:this.config.class,fields:this.validation.extractFields(this.fields),selected:this.entries.map(s=>s[this.config.mapping.value])}).subscribe({next:s=>{nsNotice.success(__m("Opertion Successful","BulkEditor"),__m("The bulk update operation was successful","BulkEditor"))},error:s=>{nsNotice.error(__m("Opertion Failed","BulkEditor"),__m("The bulk update operation failed","BulkEditor"))}}))}})}},mounted(){console.log(this.config),this.entries=this.selected,this.fields=this.validation.createFields(this.config.fields)}});nsExtraComponents.nsBulkEditor=defineComponent({name:"ns-bulk-editor",props:["selectedSubject","config"],template:`
        <div>
        ...
        `,mounted(){this.selectedSubject.subscribe({next:e=>{this.selected=e}}),this.state.subscribe({next:e=>{this.$emit("update",e)}}),console.log(this.config)},data(){return{selected:[],state:new RxJS.BehaviorSubject({})}},methods:{__m,launchEditor:function(){Popup.show(l,{selected:this.selected,config:this.config.bulkEditConfig,state:this.state})}}})});
